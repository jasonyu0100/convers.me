...
function WorkPulse() {
  ...
  const maxSteps = Math.max(...pulseData.map((item) => item.value));

  return (
    <div className='flex h-full flex-col rounded-lg border border-gray-200 bg-white/80 p-6'>
      <div className='flex flex-col items-start justify-between md:flex-row md:items-center'>
        <div>
          <h3 className='text-lg font-medium text-gray-800'>{selectedTimeFrame === 'week' ? 'Work Rhythm' : 'Quarterly Work Rhythm'}</h3>
 ...
 */
function WorkBurnup() {
  const { weeklyProgress, quarterlyProgress, selectedTimeFrame, dailyBurnup, quarterlyBurnup } = useInsight();
  ...
  return (
    <div className='flex h-full flex-col rounded-lg border border-gray-200 bg-white/80 p-6'>
      <div>
        <h3 className='text-lg font-medium text-gray-800'>{selectedTimeFrame === 'week' ? 'Weekly Progress' : 'Quarterly Progress'}</h3>
  ...
  return (
    <div className='flex h-full flex-col rounded-lg border border-gray-200 bg-white/80 p-6'>
      <div className='flex items-center justify-between'>
        <h3 className='text-lg font-medium text-gray-800'>{selectedTimeFrame === 'week' ? 'Process Tracker' : 'Quarterly Process Tracker'}</h3>
        <span className='rounded-full bg-blue-50 px-3 py-1 text-sm font-medium text-blue-700'>{completionRate.toFixed(0)}% Overall</span>
      </div>
      ...
      {topProcesses.length > 0 ? (
        <div className='space-y-4'>
          <h4 className='border-b border-gray-100 pb-1 text-sm font-medium text-gray-700'>Top Active Processes</h4>
          ...
          {sortedProcesses.length > 3 && (
            <div className='pt-2 text-center'>
              <button className='flex w-full items-center justify-center text-sm font-medium text-blue-600 hover:text-blue-800'>
