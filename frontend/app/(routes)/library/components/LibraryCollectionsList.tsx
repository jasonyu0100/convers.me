...
export function LibraryCollectionsList() {
  const { collections, setSelectedCollection } = useLibrary();
    ...
    <div className='flex-1 overflow-auto p-6'>
      ...
      <div className='grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3'>
        {collections.map((collection) => {
          // Set color based on collection id
          const collectionColor = collection.id.includes('team')
            ? 'from-purple-500 to-indigo-500'
            : collection.id.includes('personal')
            ...
            >
              ...
              <div className='flex flex-1 flex-col p-5'>
                ...
                {collection.description ? (
                  <p className='mb-auto line-clamp-2 text-sm text-slate-500'>{collection.description}</p>
                ) : (
