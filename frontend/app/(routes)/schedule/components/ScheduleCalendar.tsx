...
export function ScheduleCalendar({ onDateSelect, selectedDate, availabilityData }: CalendarProps) {
  ...
  return (
    <div className='rounded-xl bg-slate-50 p-4'>
      <div className='calendar-header mb-4 flex items-center justify-between'>
        ...
        <h2 className='text-md font-medium'>{dateUtils.formatMonthYear(currentMonth)}</h2>
        ...
        <div className='grid grid-cols-7 gap-1'>
          {calendarDays.map((date, index) => {
            const dayClasses = [
              ...
              dateUtils.isToday(date) && !isSelected(date) ? 'font-bold text-blue-500' : '',
              dateUtils.isPastDate(date) ? 'cursor-not-allowed text-slate-300' : 'cursor-pointer hover:bg-slate-100',
              hasAvailability(date) && !isSelected(date) && !dateUtils.isPastDate(date) && isCurrentMonth(date) ? 'ring-2 ring-blue-200' : '',
            ]
              .filter(Boolean)
              .join(' ');
            ...
            return (
              <div
                ...
                onClick={() => {
                  if (!dateUtils.isPastDate(date) && isCurrentMonth(date) && hasAvailability(date)) {
                    onDateSelect(date);
                  }
                }}
              >
                {date.getDate()}
                {hasAvailability(date) && !dateUtils.isPastDate(date) && isCurrentMonth(date) && (
